...
export default function AdminLayout({
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className="w-64 bg-[#0D1B2A] text-white flex flex-col shadow-xl flex-shrink-0">
        {/* Logo */}
        <div className="p-5 border-b border-white/10">
          <Link href="/" className="flex flex-col bg-white p-3 rounded-xl items-center hover:shadow-md transition-shadow">
            ...
            />
            <span className="text-[10px] font-bold text-[#0D1B2A] uppercase tracking-wider">Admin Dashboard</span>
          </Link>
        </div>
      ...
      <main className="flex-1 overflow-auto">
        <header className="bg-white border-b border-gray-200 h-16 flex items-center px-8 shadow-sm">
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            <div className="w-8 h-8 bg-[#FF6A00] rounded-full flex items-center justify-center text-white text-xs font-bold">
              A
            </div>
          </div>
        </header>
